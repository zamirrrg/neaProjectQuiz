...
class TeacherLoginWindow:
    ...
    def add_frame(self):
        ...
        self.label = Label(self.frame, text="Teacher Login")
        ...
        self.teacherpassword = Entry(self.frame,show='*', font='Courier 12')
        self.teacherpassword.place(x=200, y=y+260)

        self.button = Button(self.frame, text="Login", font='Courier 15 bold',
                             command=self.teacherlogin)
    ...
    def teacherlogin(self):
        ...
        data = (
            self.username.get(), self.teacherpassword.get()
        )

        check = db.teacher_login(data)
        ...
        if check == "Username incorrect":
            ...
            messagebox.showinfo("Password incorrect")
                ...
                self.win.destroy()
                TeachermainMenu = teachermainmenu.TeacherMainMenu(data[0])
                TeachermainMenu.add_frame()
        else: #check == False
